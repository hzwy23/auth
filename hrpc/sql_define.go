...
var (
	...
	sys_rdbms_hrpc_004 = `select org_unit_id from sys_user_info where user_id = ?`
	sys_rdbms_hrpc_005 = `select user_id,user_passwd,status_id,continue_error_cnt from sys_sec_user where user_id = ?`
	sys_rdbms_hrpc_006 = `select count(*) from sys_role_user r inner join sys_role_resource e on r.role_id = e.role_id inner join sys_theme_resource v on e.res_id = v.res_id inner join sys_user_theme m on v.theme_id = m.theme_id and r.user_id = m.user_id where r.user_id = ? and v.res_url = ?`
